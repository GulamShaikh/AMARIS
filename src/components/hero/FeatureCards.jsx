    ...
    <section className="px-6 md:px-20 py-24 bg-white">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {features.map((item, index) => (
          <div
            key={index}
            className="group rounded-2xl border border-gray-200 bg-gray-50 p-6 transition-all duration-300 hover:bg-white hover:shadow-[0_10px_30px_rgba(0,0,0,0.08)] hover:-translate-y-1"
          >
            {/* ICON */}
            <div
              className="w-10 h-10 flex items-center justify-center rounded-xl bg-white border border-gray-200 text-gray-700 mb-4 transition group-hover:bg-black group-hover:text-white"
            >
              {item.icon}
            </div>
